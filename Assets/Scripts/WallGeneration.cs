...
public static class WallGeneration
{
    ...
    {
        var basicWallPositions = FindWallsInDirections(floorPositions, Directions.Cardinal);
        var cornerWallPositions = FindWallsInDirections(floorPositions, Directions.Diagonal);



        CreateBasicWalls(visualizer, basicWallPositions, floorPositions, Directions.Cardinal);
        CreateCornerWalls(visualizer, cornerWallPositions, floorPositions, Directions.All);
    }

    private static void CreateCornerWalls(TilemapVisualizer visualizer, HashSet<Vector2Int> wallPositions, HashSet<Vector2Int> floorPositions, List<Vector2Int> directions)
    {
        foreach (var wallPosition in wallPositions)
        {
            ...
            {
                var neighbourPosition = wallPosition + dir;
                bool b = floorPositions.Contains(neighbourPosition);
                val += b ? "1" : "0";
            }
            int valToInt = Convert.ToInt32(val, 2);
            visualizer.PaintSingleCornerWall(wallPosition, valToInt);
        }
    }

    private static void CreateBasicWalls(TilemapVisualizer visualizer, HashSet<Vector2Int> wallPositions, HashSet<Vector2Int> floorPositions, List<Vector2Int> directions)
    {
        foreach (var wallPosition in wallPositions)
        {
            string val = "";
            ...
            int valToInt = Convert.ToInt32(val, 2);
            visualizer.PaintSingleBasicWall(wallPosition, valToInt);
        }
    }
